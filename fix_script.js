...
const htmlPath = 'c:\\Users\\grego\\Documents\\LAN-demain\\LAN-demain\\index.html';
const html = fs.readFileSync(htmlPath, 'utf8');

const newScript = `        const normalizeGameName = (name) => {
...
if (startIdx !== -1 && endIdx !== -1) {
    const configScriptIdx = html.indexOf('<script src="config.js"></script>');
    const actualStart = html.indexOf('<script>', configScriptIdx + 10);
    const actualEnd = html.indexOf('</script>', actualStart);

    const newHtml = html.substring(0, actualStart + 8) + '\n' + newScript + '\n    ' + html.substring(actualEnd);
    fs.writeFileSync(htmlPath, newHtml);
    console.log('Successfully updated index.html script block');
} else {
